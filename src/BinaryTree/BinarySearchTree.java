...
public class BinarySearchTree<ELEMENT extends Comparable<ELEMENT>> extends BinaryTree<ELEMENT> {
    ...
    public BTNode<ELEMENT> search(ELEMENT key) {
        return searchRecursive(this.root, key);
    }

    private BTNode<ELEMENT> searchRecursive(BTNode<ELEMENT> current, ELEMENT key) {
        if (current == null || current.item.compareTo(key) == 0) {
            return current;
        }
